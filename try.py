"""
cross_model_evaluation_normalized.py
─────────────────────────────────────
Extends the LOO evaluation with:
  1. Rank-normalization of features AND target per model
  2. Per-dataset-family regressors (Wiki / OWT)
  3. Side-by-side Raw vs Normalized LOO chart
  4. Cross-model generalization chart (Wiki→OWT, OWT→Wiki, family→GPT-J/OLMo stubs)
...
results_raw  = {}               # LOO results, raw features
results_norm = {}               # LOO results, normalized features
rf_imp_raw   = {}               # permutation importances (raw)
...
def train_loo(model_subset=None):
    ...
    for loo_model in targets:
        ...
        if not common_feats:
            ...
            results_raw[loo_model] = results_norm[loo_model] = None
            continue

        # ── Raw ───────────────────────────────────────────────
        X_tr_r = np.concatenate(
            [global_dataframes[m]['df'][common_feats].values for m in train_models])
        y_tr_r = np.concatenate(
            [global_dataframes[m]['y'].values for m in train_models])
        X_te_r = global_dataframes[loo_model]['df'][common_feats].values
        y_te_r = global_dataframes[loo_model]['y'].values

        print(f"  [LOO raw ] {loo_model} ← {len(train_models)} models, "
              f"{len(common_feats)} feats")
        results_raw[loo_model] = _fit_and_evaluate(
            X_tr_r, y_tr_r, X_te_r, y_te_r, common_feats, rf_imp_raw)
        r = results_raw[loo_model]
        if r:
...
# ──────────────────────────────────────────────────────────────
# CHART 1  –  Raw vs Normalized LOO (side-by-side)
# ──────────────────────────────────────────────────────────────

def _bar_group(ax, labels, raw_dict, norm_dict, title):
    """Helper: draw one LOO panel (raw or norm) as a grouped bar chart."""
    lin_r = [raw_dict[m]['lin_r2'] if raw_dict.get(m) else 0  for m in labels]
    mlp_r = [raw_dict[m]['mlp_r2'] if raw_dict.get(m) else 0  for m in labels]
    rf_r  = [raw_dict[m]['rf_r2']  if raw_dict.get(m) else 0  for m in labels]
    ...
    vals    = [lin_r, mlp_r, rf_r, lin_n, mlp_n, rf_n]
    colors  = [C_LIN, C_MLP, C_RF,
               C_LIN, C_MLP, C_RF]
    hatches = ['', '', '', '///', '///', '///']
    bar_labels = ['Lin (raw)', 'MLP (raw)', 'RF (raw)',
                  'Lin (norm)', 'MLP (norm)', 'RF (norm)']

    bars_all = []
    for offset, val, color, hatch, lbl in zip(
            offsets, vals, colors, hatches, bar_labels):
...
def generate_loo_comparison_chart():
    valid = [m for m in MODEL_SIZES
             if results_raw.get(m) or results_norm.get(m)]
    if not valid:
        print("No LOO results to plot.")
        return

    fig, ax = plt.subplots(figsize=(20, 7))
    _bar_group(ax, valid, results_raw, results_norm,
               "LOO Generalization: Raw Features vs Rank-Normalized Features\n"
               "(Hatched = normalized; solid = raw)")

    plt.tight_layout()
    fname = "loo_raw_vs_normalized.png"
...
# ──────────────────────────────────────────────────────────────
# CHART 3  –  RF Feature Importance (raw vs normalized)
# ──────────────────────────────────────────────────────────────

def generate_feature_importance_chart():
    if not rf_imp_raw and not rf_imp_norm:
        print("No importance data.")
        return

    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    for ax, imp_store, title in [
        (axes[0], rf_imp_raw,  "RF Permutation Importance — Raw Features"),
        (axes[1], rf_imp_norm, "RF Permutation Importance — Normalized Features"),
    ]:
        if not imp_store:
        ...
        ax.set_xlabel("Mean decrease in R² (permutation, held-out test)", fontsize=9)
        ax.set_title(title, fontweight='bold', fontsize=11)
    ...
    plt.suptitle("Random Forest Feature Importance: Raw vs Normalized",
                 fontsize=13, fontweight='bold')
    plt.tight_layout()
    fname = "rf_feature_importance_comparison.png"
    plt.savefig(fname, dpi=300)
    print(f"Saved '{fname}'")
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    # ── Step 1: Full LOO (raw + normalized) ──────────────────
    print("\n── LOO evaluation (raw + normalized) ───────────────────")
    train_loo()
