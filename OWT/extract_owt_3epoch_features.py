...
OUTPUT_CSV = os.path.join(CURRENT_DIR, "owt_3epoch_features.csv")
...
# CRITICAL: Same dims as Wiki Probability Extraction
BATCH_SIZE = 64      
BLOCK_SIZE = 128
MAX_BATCHES = 1000 
NUM_WORKERS = 4      
...
class OWTDataset(Dataset):
    def __init__(self, cache_path, block_size=128):
        ...
        if os.path.exists(cache_path):
            print(f"🚀 Loading OWT tokens from: {cache_path}")
...
def get_fixed_batches(loader, max_batches):
    print(f"🔒 Freezing {max_batches} batches of OWT...")
    fixed_data = []
...
def get_step_from_filename(filename):
    # UPDATED: Robust regex that extracts the global step number from both:
    # 'ckpt_step_500.pt' AND 'ckpt_step_28759_epoch_1.pt'
...
def get_all_checkpoints_sorted(folder_path):
    ...
    # SORTING PARITY: Sorted strictly by the extracted integer global step
...
def extract_all_features(model, fixed_batches, device):
    ...
    # 5. Average Probability over 1000 batches
    all_probs = []
    with torch.no_grad():
...
def main():
    ...
    print(f"🖥️  Running Unified Extraction on: {device}")
    ...
    for model_size, conf in CONFIGS.items():
        folder_name = f"{model_size}_owt_3epoch"
    ...
    if master_results:
        ...
        print(f"\n🎉 SUCCESS! Unified OWT Master dataset saved to: {OUTPUT_CSV}")
