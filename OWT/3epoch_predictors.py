...
OUTPUT_DIR = os.path.join(CURRENT_DIR, "3epoch_graphs", "experiment_results")
os.makedirs(OUTPUT_DIR, exist_ok=True)

WIKI_FILE = os.path.join(CURRENT_DIR, "wiki_3epoch_features.csv")
OWT_FILE = os.path.join(CURRENT_DIR, "owt_3epoch_features.csv")
...
def run_cross_dataset_zero_shot():
    ...
    # =================================================================================
    # CHANGED: ACADEMIC SORTING LOGIC & TOP 10 TRUNCATION
    # Sorts primarily by R² descending (-x['r2']), and secondarily by MAE ascending (x['mae'])
    # =================================================================================
    print(f"\n{'='*80}\n🏆 GLOBAL ZERO-SHOT MODEL RANKING (ACADEMIC SORT: BY HIGHEST R²)\n{'='*80}")
    final_results.sort(key=lambda x: (-x['r2'], x['mae']))
    
    print(f"{'Rank':<5} | {'Scenario (Model)':<55} | {'MAE':<10} | {'R²':<8}")
    print("-" * 90)
    for idx, item in enumerate(final_results[:10], 1): # Restricted loop slice to exactly top 10
    ...
    for idx, item in enumerate(final_results[:10], 1):
        if item['model_name'] == 'Random Forest' and item['importances'] is not None:
            ...
            filename_fi = filename_fi.replace(' ', '_')
