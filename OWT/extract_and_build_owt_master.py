...
OUTPUT_CSV = os.path.join(CURRENT_DIR, "owt_master_features.csv")
...
def get_step_from_filename(filename):
    ...
    if 'final' in filename:
        return 29000  # Based on the Wiki assumption
    return None
    ...
    for model_size, conf in CONFIGS.items():
        ...
        for step, ckpt_path in valid_ckpts:
            try:
                ...
                print(f"   ✅ Step {step:<5} | Emb: {emb_norm:.1f} | L1: {l1_norm:.1f} | Var: {variance:.5f} | Logit: {logit_norm:.1f} | Prob: {avg_prob:.4f}")
