...
def run_all_micro_tests():
    ...
    for dataset_name, matrix_path in MATRICES.items():
        ...
        # -----------------------------------------------------------------
        # TEST 1: RANDOM 80/20 SPLIT (DATA LEAKAGE EXPECTED)
        # -----------------------------------------------------------------
        print("\n--- TEST 1: RANDOM 80/20 SPLIT (DATA LEAKAGE EXPECTED) ---")
        for model_name in models:
            ...
            # Moved Scaler OUTSIDE the algorithm loop for efficiency
            ...
            for algo_name, algo in algorithms.items():
                ...
                if model_name == '124M':
                    # Safe dictionary initialization and assignment
        ...
        # -----------------------------------------------------------------
        # TEST 2: GROUPED 80/20 SPLIT (CLEAN EVALUATION)
        # -----------------------------------------------------------------
        print("\n--- TEST 2: GROUPED BY CHECKPOINT (CLEAN EVALUATION) ---")
        for model_name in models:
            df_model = df[df['Model'] == model_name].copy()
            if df_model.empty: continue
            X, Y = df_model[feature_cols].values, df_model['Step'].values
